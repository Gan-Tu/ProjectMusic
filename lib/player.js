...
export function PlayerProvider({ children }) {
  ...
  const setResumeTime = useCallback((time) => {
    const pending = live.current.resume;
    if (!pending) return;
    const next = { ...pending, time };
  ...
  const showVideoSession = useCallback(
    (video) =>
      setMediaSession(
        ...
        {
          // Once the video is gone (another page), the controls go back to the music.
  ...
  }, [showMusicSession, showVideoSession]);
