...
function reducePlayer(state, action) {
  switch (action.type) {
    ...
    case "resume/set":
      return state.resume ? { ...state, resume: { ...state.resume, time: action.time } } : state;
    case "resume/clear":
...
export function PlayerProvider({ children }) {
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ queue, index, volume, muted, repeat, shuffle })
      );
    } catch {
      // storage unavailable
    }
  }, [hydrated, queue, index, volume, muted, repeat, shuffle]);
  ...
  useEffect(() => {
    ...
    const savePosition = () => {
      ...
      // Nothing loaded yet, or still waiting to resume this track: keep what's saved.
      if (!trackId || audio.readyState < 1 || pending?.trackId === trackId) return;
      lastSaved = Date.now();
      try {
        window.localStorage.setItem(
          POSITION_KEY,
          JSON.stringify({ trackId, time: audio.currentTime, duration: audio.duration || 0 })
        );
      } catch {
        // storage unavailable
      }
    ...
    const onMeta = () => {
      ...
      if (pending?.trackId === loadedTrackId.current) {
        if (pending.time < audio.duration - 2) audio.currentTime = pending.time;
  ...
  const seek = useCallback((seconds) => {
    ...
    if (pending && audio.readyState < 1) {
      // Not loaded yet: move the saved position instead (applied once it loads).
      dispatch({ type: "resume/set", time: Math.max(0, Math.min(seconds, pending.duration)) });
      return;
    }
  ...
  const seekBy = useCallback(
    (delta) => {
      const audio = audioRef.current;
      const pending = live.current.resume;
      const current = pending && audio?.readyState < 1 ? pending.time : audio?.currentTime || 0;
      seek(current + delta);
    },
    [seek]
  );
