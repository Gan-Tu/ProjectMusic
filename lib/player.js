...
//
// Until the listener picks something, the queue is the site's default (the title
// tracks of the home page's new releases, /api/public/default-queue), fetched after
// load and after a reset so CRM edits apply. It starts (and may stay) empty: the bar
// then asks the listener to pick something. A queue the listener changed is kept.
...
const DEFAULTS = {
  ...
  repeat: "off",
  shuffle: false,
  // The queue is the site's default (nothing picked yet): refreshed on every visit.
  isDefaultQueue: true,
  // Saved position to resume from once the track loads: { trackId, time, duration }.
  resume: null
};
// Actions that make the queue the listener's own.
const USER_QUEUE_ACTIONS = ["play", "adopt", "enqueue", "jump", "remove"];
...
// Embedded YouTube / Vimeo players (components/videos/VideoEmbed.js) take commands
// through postMessage.
function pauseEmbeds() {
  document.querySelectorAll("iframe[data-video-embed]").forEach((frame) => {
    const youtube = frame.dataset.videoEmbed === "youtube";
...
function reducePlayer(state, action) {
  switch (action.type) {
    ...
    case "play": {
      ...
      if (!state.queue.length) return { ...state, queue: [action.track], index: 0 };
      const queue = [...state.queue];
      queue.splice(state.index + 1, 0, action.track);
      return { ...state, queue, index: state.index + 1 };
    }
    // The site's default queue, fetched after load (see loadDefaultQueue): replaces
    // the placeholder unless the listener picked something meanwhile.
    case "default-queue": {
      if (!state.isDefaultQueue) return state;
      const current = state.queue[state.index]?.id;
      const index = Math.max(
        0,
        action.queue.findIndex((t) => t.id === current)
      );
      return { ...state, queue: action.queue, index };
    }
    // A fuller copy of the queue that's playing (e.g. an album's full tracklist, loaded
    // after its first track started): swapped in without interrupting the track.
    case "adopt": {
      if (!sameQueue(state.queue, action.from)) return state;
      const index = action.queue.findIndex((t) => t.id === state.queue[state.index]?.id);
      return index < 0 ? state : { ...state, queue: action.queue, index };
    }
    case "enqueue":
...
function playerReducer(state, action) {
  let next = reducePlayer(state, action);
  if (next !== state && next.isDefaultQueue && USER_QUEUE_ACTIONS.includes(action.type)) {
    next = { ...next, isDefaultQueue: false };
  }
...
export function PlayerProvider({ children }) {
  ...
  const [state, dispatch] = useReducer(playerReducer, { ...DEFAULTS, hydrated: false });
  const { queue, index, volume, muted, repeat, shuffle, isDefaultQueue, resume, hydrated } = state;
  ...
  const live = useRef({ queue, index, repeat, shuffle, resume, isDefaultQueue });
  useEffect(() => {
    live.current = { queue, index, repeat, shuffle, resume, isDefaultQueue };
  }, [queue, index, repeat, shuffle, resume, isDefaultQueue]);
  ...
  // The site's default queue (possibly empty; never autoplays; a pick made meanwhile
  // wins, and nothing changes under a track that's already playing).
  const loadDefaultQueue = useCallback(() => {
    fetch("/api/public/default-queue")
      .then((response) => (response.ok ? response.json() : Promise.reject(response.status)))
      .then(({ tracks }) => {
        if (!Array.isArray(tracks) || wantPlay.current) return;
        dispatch({ type: "default-queue", queue: tracks });
      })
      .catch(() => {}); // offline: the queue stays as it is
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (validQueue) {
        ...
        payload.isDefaultQueue = saved.isDefaultQueue === true;
      }
    ...
    if (payload.isDefaultQueue !== false) loadDefaultQueue();
  }, [loadDefaultQueue]);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const current = { queue, index, volume, muted, repeat, shuffle, isDefaultQueue };
    ...
    if ("queue" in changed || "index" in changed) {
      Object.assign(changed, { queue, index, isDefaultQueue });
    }
    ...
    withPlayerLock(() => {
      ...
      try {
        ...
        if (trackChanged) {
          // Drop a position saved for another track (a seek on the new one may have
          // been saved already while this waited for the lock).
          const position = JSON.parse(window.localStorage.getItem(POSITION_KEY) || "null");
          if (position?.trackId !== current.queue[current.index]?.id) {
  ...
  }, [hydrated, queue, index, volume, muted, repeat, shuffle, isDefaultQueue, applyReset]);
  ...
  const claimPlayback = useCallback(() => {
    ...
    const { queue: q, index: i, isDefaultQueue: d } = live.current;
    withPlayerLock(() => {
      const stored = readPlayerState();
      if ((stored.value?.resets || 0) > resetGen.current) return; // a reset elsewhere wins
      const saved = { ...stored.value, queue: q, index: i, isDefaultQueue: d };
      try {
        window.localStorage.setItem(
          STORAGE_KEY,
          JSON.stringify({ ...saved, resets: resetGen.current })
        );
      } catch {
        // storage unavailable
      }
  ...
  useEffect(() => {
    ...
    if (!audio) return;
    if (!src) {
      // Nothing to play (an empty queue): unload the last track.
  ...
  const play = useCallback(() => {
    if (!live.current.queue[live.current.index]) return; // empty queue
  ...
  const seek = useCallback(
    (seconds) => {
      ...
      if (audio.readyState < 1 && current) {
        // Not loaded and nothing to resume yet: this seek becomes the position to
        // start from (saved now, applied once it loads), using the listed duration.
        const duration = current.duration || seconds;
        const next = {
          trackId: current.id,
        ...
        live.current = { ...live.current, resume: next };
        dispatch({ type: "resume/begin", resume: next });
        claimPlayback();
        writePosition(next);
        return;
      }
  ...
  // Only one thing plays at a time: the music starting pauses any playing video (and
  // embedded players), and a video starting pauses the music ("play" doesn't bubble,
  // so capture it).
  useEffect(() => {
    const audio = audioRef.current;
    const onMediaPlay = (e) => {
  ...
  const value = useMemo(
    () => ({
      ...
      // Replaces the queue `from` (still playing) with `queue`, keeping the track.
      adoptQueue: (queue, from) => dispatch({ type: "adopt", queue, from }),
      jumpTo: (i) => {
