...
// load and after a reset so CRM edits apply; DEFAULT_QUEUE is only the placeholder
// shown before that answer arrives. A queue the listener changed is kept as saved.
...
export function PlayerProvider({ children }) {
  ...
  // The site's default queue, in place of the placeholder (never autoplays; a pick
  // made meanwhile wins, and nothing changes under a track that's already playing).
  const loadDefaultQueue = useCallback(() => {
    fetch("/api/public/default-queue")
      .then((response) => (response.ok ? response.json() : Promise.reject(response.status)))
      .then(({ tracks }) => {
        if (!Array.isArray(tracks) || !tracks.length || wantPlay.current) return;
