  ...
  switch (action.type) {
    ...
    case "comment/edit":
      return {
        ...
        comments: {
          ...state.comments,
          // Only over the text the edit started from (never over someone else's save).
          [action.threadId]: (state.comments[action.threadId] || []).map((c) =>
            c.id === action.id && (action.expected === undefined || c.text === action.expected)
              ? { ...c, text: action.text, editedAt: action.at }
              : c
          )
        }
      };
...
export function StoreProvider({ children }) {
    ...
    (method, explicitItems, confirmedCart) =>
      withStoreLock(() => {
        ...
        const purchase = {
          ...
          creditsGranted,
          // 10 points per dollar on card (from cents, so no rounding drift), 1 per 2 credits.
          pointsEarned: Math.round(method === "card" ? totals.cents / 10 : totals.credits / 2)
        };
  ...
  // Saves a comment edit made from `expected` (the text the editor opened with), checked
  // against the latest saved comment under the cross-tab lock. Resolves to { ok },
  // { conflict, text } when it was changed meanwhile, or { error }.
  const editComment = useCallback(
    (threadId, id, text, expected) =>
      withStoreLock(() => {
        const stored = readStored();
        const batch = pending.current.slice();
        const current = rebase(stored.data, batch);
        const comment = (current.comments[threadId] || []).find((c) => c.id === id);
        if (!comment) return { ok: false, error: "This comment was deleted." };
        if (comment.text !== expected) return { ok: false, conflict: true, text: comment.text };
        const action = {
          type: "comment/edit",
          threadId,
          id,
        ...
        const data = pickPersisted(reducer(current, action));
        if (writeStored(stored.rev + 1, data)) {
          pending.current = pending.current.slice(batch.length);
          showCommitted(data);
        } else {
  ...
  const actions = useMemo(
    ...
    [dispatch, checkout, editComment, sendMessage, resetDemo, makeUndo]
  );
