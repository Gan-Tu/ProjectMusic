  ...
  const flush = useCallback(
    () =>
      withStoreLock(() => {
        ...
        if (JSON.stringify(stored.data) !== JSON.stringify(data)) writeStored(stored.rev + 1, data);
    ...
    (method, explicitItems) =>
      withStoreLock(() => {
        ...
        const data = pickPersisted(
          reducer(current, { type: "purchase", purchase, fromCart: !explicitItems })
        );
        writeStored(stored.rev + 1, data);
        pending.current = pending.current.slice(batch.length);
        showCommitted(data);
