...
function toEvent(row) {
  ...
  return {
    ...
    // null without tiers (tickets not on sale yet).
    price: tiers[0]?.price ?? null,
    credits: tiers[0]?.credits ?? null,
...
function toProduct(row) {
  const product = {
    ...
    price: row.price,
    credits: row.credits,
    // Without images: the ticket artwork for tickets, the Projct Music card otherwise.
    images: list(row.images).length
      ? row.images
      : [row.kind === "ticket" ? "/shop/ticket.webp" : "/shop/digital.svg"],
    description: row.description,
  ...
  if (row.event_id) product.eventId = row.event_id;
  // Tickets take their prices, tiers and sales end from the event; without an event or
  // tiers they can't be bought yet ("Tickets coming soon").
  if (row.kind === "ticket") {
    const tiers = row.event_id ? toTiers(row.event_tiers) : [];
