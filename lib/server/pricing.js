...
function unavailable(name) {
  return new HttpError(409, `${name || "An item in your order"} is no longer available.`, {
    code: "unavailable"
  });
}
...
function priceTicket(line, event, now) {
  if (!event || event.status !== "published") throw unavailable(line.name);
  ...
  if (Object.keys(line.options).some((key) => key !== "tier")) throw changed(event.title);
  const tiers = eventTiers(event.tiers);
  if (!tiers.length) throw unavailable(`Tickets for ${event.title}`);
  ...
  if (!tier) throw changed(`The ${line.options.tier} ticket for ${event.title}`);
  return {
    id: `ticket:${event.id}`,
    name: event.title,
...
function expectedLine(line, kind, catalog, now) {
  if (kind.type === "pack") {
    ...
    if (!pack || Object.keys(line.options).length) throw unavailable(line.name);
  ...
  if (kind.type === "album") {
    ...
    if (!album || !tier || album.status !== "published") throw unavailable(line.name);
    return tierLine(
      albumPurchaseItem({
        id: album.id,
        name: album.name,
  ...
  if (!row || row.status !== "published") throw unavailable(line.name);
  ...
  if (Object.keys(other).length) throw changed(product.name);
  const tier = tierLabel ? product.tiers.find((t) => t.label === tierLabel) : undefined;
  if (tierLabel && !tier) throw changed(`The ${tierLabel} option of ${product.name}`);
  if (size && !product.sizes.includes(size)) throw changed(`Size ${size} of ${product.name}`);
  if (color && !product.colors.includes(color)) throw changed(`${color} ${product.name}`);
  ...
  if (!sameOptions(expected.options, line.options)) throw changed(product.name);
  if (expected.price == null && expected.credits == null) throw unavailable(product.name);
  return expected;
}
