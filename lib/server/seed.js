...
const CLEAR_MEMBERS = [
  ...
  "delete from orders",
  "delete from inbox",
  "delete from sessions where kind = 'user'", // never the CRM admin's sessions
  "delete from users"
];
...
export async function applySeed({ scope = "content" } = {}) {
  ...
  const clear = [
    // Overlapping resets (a double-clicked button) run one after the other.
    "select pg_advisory_xact_lock(hashtext('projct-music:seed'))",
    ...[...CONTENT_TABLES].reverse().map((table) => `delete from ${table}`),
    ...(scope === "all" ? CLEAR_MEMBERS : [])
  ];
  return withTransaction(async (client) => {
    await client.query(clear.join(";\n")); // one round trip (simple query protocol)
    const counts = {};
    for (const table of tables) counts[table] = await insertRows(client, table, data[table]);
    await client.query(
      "insert into audit_log (actor, action, entity, detail) values ('system', 'reset', 'database', $1::jsonb)",
