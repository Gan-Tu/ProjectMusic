...
import { getMusics, albumToTrack } from "../utils/getFakeTracks";
import { getArtistHomePageData } from "../utils/getFakeArtistsData";

export default function Home({ videos, albums, queue, artists }) {
  return (
    <AppContainer
      title="Home"
      curMenu="Home"
    ...
    >
      ...
      <NewReleases albums={albums} queue={queue} />
...
export function getStaticProps() {
  const albums = getMusics().slice(0, 12);
  const videos = getVideos();
  const artistIds = videos.map((video) => video.artistId);
  return {
    props: {
      videos,
      albums,
      queue: albums.map(albumToTrack),
      artists: getArtistHomePageData()
